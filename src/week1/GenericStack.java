...
public class GenericStack<E> extends Assert implements Stack<E>  {
	...
	public void pushAll(Collection<? extends E> src) throws StackException {
		if(src.size() > capacity - N) {
			...
			throw new StackException("Size of the source is greater than stack can handle");
		}
		for (E element : src) {
			push(element);
		}
	...
	public void popAll(Collection<? super E> dst) throws StackException {		
		if (dst.size() < N) {
			...
			throw new StackException("Size of the destination is less than quantity of elements in stack");
		}
		while (!isEmpty()) {
			dst.add(pop());
		}
