...
public class ModelImpl implements Model {
	...
	public boolean put (Account account, double sum) {
		if (!this.authorized()) return false;
		if (!(account.getOwner().equals(currentUser.getName()))) return false;
		if (sum <= 0) return false;
		account.changeBalance(sum);
		account.addRecord(new Record(account, "adding money", sum, new Date(), Tags.PUT));
		return true;
	}
	...
	public boolean withdraw (Account account, double sum) {
		if (!this.authorized()) return false;
		if (!account.getOwner().equals(currentUser.getName())) return false;
		if (sum <= 0 || account.getBalance() < sum) return false;
		account.changeBalance(-sum);
		account.addRecord(new Record(account, "withdrawing money", sum, new Date(), Tags.WITHDRAW));
		return true;
	}
	...
	public boolean createAccount(String description) {
		if (!this.authorized()) return false;
		data.addAccount(currentUser, new Account(description, currentUser.getName()));
		return true;
	}
